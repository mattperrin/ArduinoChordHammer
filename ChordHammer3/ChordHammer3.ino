  ...
  if (differenceTiming >= sixteenInterval)
  {
    switch (playMode1)
    {
      case 0:
        //STACCATO
        if (Seq1On) {
          MIDI.sendNoteOff(Seq11Pitch, Seq11Velocity, midiChannel);
          Seq1On = false;
        }
        if (Seq2On) {
          MIDI.sendNoteOff(Seq12Pitch, Seq12Velocity, midiChannel);
          Seq2On = false;
        }
        if (Seq3On) {
          MIDI.sendNoteOff(Seq13Pitch, Seq13Velocity, midiChannel);
          Seq3On = false;
        }

        if (CheckSequenceByte(1, intervalCounter) == true)
        {
          MIDI.sendNoteOn(Seq11Pitch, Seq11Velocity, midiChannel);
          Seq1On = true;
        }
        if (CheckSequenceByte(2, intervalCounter) == true)
        {
          MIDI.sendNoteOn(Seq12Pitch, Seq12Velocity, midiChannel);
          Seq1On7 = true;
        }
        if (CheckSequenceByte(3, intervalCounter) == true)
        {
          MIDI.sendNoteOn(Seq13Pitch, Seq13Velocity, midiChannel);
          Seq1On = true;
        }
        break;
